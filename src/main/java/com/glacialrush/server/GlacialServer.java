...
public class GlacialServer extends GlacialPlugin implements Listener
{
	...
	public void onEnable()
	{
		...
		getCommand(Info.CMD_PING).setExecutor(commandController);
		getCommand(Info.CMD_HUB).setExecutor(commandController);
		getCommand(Info.CMD_ACTION).setExecutor(commandController);
	...
	public void placeBounty(final Player p, final GList<Player> bounties, final RegionedGame rg)
	{
		final Pane pane = new Pane(getUiController().get(p), ChatColor.LIGHT_PURPLE + "Select a Player.");
		final GList<Player> unbounties = new GList<Player>();
